...
class CostEvaluation:
    ...
    def calculate_cost(self, workload, indexes, store_size=False):
        ...
        logging.debug("\nCalculating cost for workload:")
        logging.debug(f"  Indexes: {[str(i) for i in indexes]}")
        
        # TODO: Make query cost higher for queries which are running often
        for query in workload.queries:
            self.cost_requests += 1
            query_cost = self._request_cache(query, indexes)
            logging.debug(f"  Query {query.nr}:")
            logging.debug(f"    Text: {query.text}")
            logging.debug(f"    Cost: {query_cost:.2f}")
            total_cost += query_cost
            
        logging.debug(f"  Total cost: {total_cost:.2f}")
        return total_cost
