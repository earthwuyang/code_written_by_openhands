...
class WhatIfIndexCreation:
    ...
    def simulate_index(self, potential_index, store_size=False):
        logging.debug(f"\nSimulating index: {potential_index}")
        logging.debug(f"  Table: {potential_index.table()}")
        logging.debug(f"  Columns: {[str(c) for c in potential_index.columns]}")
        ...
        logging.debug(f"  HypoPG OID: {index_oid}")
        logging.debug(f"  HypoPG Name: {index_name}")

        if store_size:
            size = self.estimate_index_size(index_oid)
            potential_index.estimated_size = size
            logging.debug(f"  Estimated size: {size} bytes")
