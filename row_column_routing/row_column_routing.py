...
def main():
    # Load data
    data_dir = '/home/wuy/AI/MLR-Copilot/workspaces/row_column_routing/data/tpch_sf1'
    csv_file = os.path.join(data_dir, 'query_costs.csv')
    plan_dir = os.path.join(data_dir, 'row_plans')
    ...
    for query_id in tqdm(df['query_id'], desc='Processing query plans'):
        with open(os.path.join(plan_dir, f'{query_id}.json'), 'r') as f:
            plan = json.load(f)
