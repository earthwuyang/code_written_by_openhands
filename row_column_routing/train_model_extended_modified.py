...
def calculate_mean(values):
    ...
    count = 0
    for value in values:
        # if value is nan, value = 0
        # print(f"value: {value}")
        if np.isnan(value):
            value = 0
        sum = sum * (count / (count + 1)) + value / (count + 1)
        count = count + 1
    print(f"mean: {sum}")
    return sum
...
def calculate_end_to_end_time(predictions, actual_labels, row_times, column_times):
    ...
    actual_labels = [int(i) for i in actual_labels]
    print(f"predictions: {predictions[:10]}")
    print(f"actual_labels: {actual_labels[:10]}")
    print(f"row_times: {row_times[:10]}")
    print(f"column_times: {column_times[:10]}")

    ai_total_time = calculate_mean([row_t if pred == 0 else col_t for pred, row_t, col_t in zip(predictions, row_times, column_times)])
    optimal_total_time = calculate_mean([min(row_t, col_t) for row_t, col_t in zip(row_times, column_times)])
    actual_total_time = calculate_mean([row_t if label == 0 else col_t for label, row_t, col_t in zip(actual_labels, row_times, column_times)])
...
def main():
    ...
    df = df[:1000]
    print(f"Original number of queries: {len(df)}")
    df=df.dropna(subset=['row_time'])

    # Reset index so that indices are continuous from 0 to n-1
    ...
    # Extract features from both row and column plans
    X_train = X_combined[train_df.index]
    X_val = X_combined[val_df.index]
    X_test = X_combined[test_df.index]
    y_train = train_df['use_imci'].values
    y_val = val_df['use_imci'].values
    y_test = test_df['use_imci'].values

    test_row_times = test_df['row_time'].values
    test_column_times = test_df['column_time'].values
    ...
    cost_threshold_preds = [cost_threshold_router(qid) for qid in test_df['query_id']]
